...
import (
	...
	"github.com/samnoadd/NookClaw/pkg/auth"
	"github.com/samnoadd/NookClaw/pkg/config"
	"github.com/samnoadd/NookClaw/web/backend/utils"
)
...
func newRemoteGatewayTestConfig() *config.Config {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.Provider = ""
	cfg.Agents.Defaults.ModelName = "remote-default"
	cfg.Agents.Defaults.Model = ""
	cfg.ModelList = []config.ModelConfig{{
		ModelName: "remote-default",
		Model:     "openai/gpt-4o",
		APIKey:    "test-key",
	}}
	return cfg
...
func TestGatewayStartReady_ValidDefaultModel(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.json")
	cfg := newRemoteGatewayTestConfig()
	err := config.SaveConfig(configPath, cfg)
	if err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-nookclaw")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("NOOKCLAW_BINARY", invalidBinaryPath)
...
func TestFindNookclawBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "nookclaw-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("NOOKCLAW_BINARY", mockBinary)

	got := utils.FindNookclawBinary()
	if got != mockBinary {
		t.Errorf("FindNookclawBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindNookclawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When NOOKCLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("NOOKCLAW_BINARY", "/nonexistent/nookclaw-binary")

	got := utils.FindNookclawBinary()
	// Should not return the invalid path; falls back to "nookclaw" or another found path
	if got == "/nonexistent/nookclaw-binary" {
		t.Errorf("FindNookclawBinary() returned invalid env path %q, expected fallback", got)
	}
}
