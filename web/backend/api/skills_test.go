...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("NOOKCLAW_BUILTIN_SKILLS")
	if err := os.Setenv("NOOKCLAW_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(NOOKCLAW_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("NOOKCLAW_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("NOOKCLAW_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
