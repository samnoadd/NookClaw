...
const (
	autoStartEntryName = "NookClawLauncher"
	launchAgentLabel   = "io.nookclaw.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "nookclaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=NookClaw Web",
			"Comment=Start NookClaw Web on login",
