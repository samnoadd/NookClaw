...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("NOOKCLAW_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("NOOKCLAW_HOME", filepath.Join(tmp, ".nookclaw")); err != nil {
		t.Fatalf("set NOOKCLAW_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("NOOKCLAW_HOME")
		} else {
			_ = os.Setenv("NOOKCLAW_HOME", oldPicoHome)
		}
	}
