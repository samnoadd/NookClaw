...
func NewJSONLStore(dir string) (*JSONLStore, error) {
	...
	if err := os.Chmod(dir, 0o700); err != nil {
		return nil, fmt.Errorf("memory: chmod directory: %w", err)
	}
...
func (s *JSONLStore) addMsg(sessionKey string, msg providers.Message) error {
	...
	if chmodErr := os.Chmod(s.jsonlPath(sessionKey), 0o600); chmodErr != nil {
		return fmt.Errorf("memory: chmod jsonl: %w", chmodErr)
	}
