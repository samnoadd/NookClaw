...
import (
	...
	"github.com/samnoadd/NookClaw/pkg/constants"
	"github.com/samnoadd/NookClaw/pkg/fileutil"
...
const (
	...
	heartbeatTaskMarker    = "Add your heartbeat tasks below this line:"
)
...
func (hs *HeartbeatService) buildPrompt() string {
	...
	if !hasConfiguredHeartbeatTasks(content) {
		return ""
	}
...
Only tasks written below the divider are actionable.
Everything above the divider is documentation and must never be treated as a task.

## Instructions

- Leave the task section empty to disable autonomous actions.
- Do not infer tasks from examples, comments, old runs, or agent memory.
- Run only the tasks explicitly listed below the divider.
- If nothing is listed below the divider, respond with HEARTBEAT_OK.
...
`
	...
	if err := fileutil.WriteFileAtomic(heartbeatPath, []byte(defaultContent), 0o600); err != nil {
...
func hasConfiguredHeartbeatTasks(content string) bool {
	trimmed := strings.TrimSpace(content)
	if trimmed == "" {
		return false
	}

	idx := strings.Index(content, heartbeatTaskMarker)
	if idx == -1 {
		return trimmed != ""
	}

	tasks := strings.TrimSpace(content[idx+len(heartbeatTaskMarker):])
	return tasks != ""
}
