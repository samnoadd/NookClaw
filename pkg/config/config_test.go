...
// TestDefaultConfig_HeartbeatDisabled verifies heartbeat is disabled by default
...
func TestDefaultConfig_Model(t *testing.T) {
	...
	if cfg.Agents.Defaults.ModelName != "private-local" {
		t.Errorf("ModelName = %q, want private-local", cfg.Agents.Defaults.ModelName)
	}
}
...
func TestSaveConfig_PersistsDefaultModelNameAndOmitsDeprecatedModelField(t *testing.T) {
	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "config.json")
	...
	if !strings.Contains(string(data), `"model_name": "private-local"`) {
		t.Fatalf("saved config should include the default model_name, got: %s", string(data))
	}
	if strings.Contains(string(data), `"model": ""`) {
		t.Fatalf("saved config should omit the deprecated empty model field, got: %s", string(data))
	}
}
...
func TestDefaultConfig_OpenAIWebSearchDisabled(t *testing.T) {
	cfg := DefaultConfig()
	if cfg.Providers.OpenAI.WebSearch {
		t.Fatal("DefaultConfig().Providers.OpenAI.WebSearch should be false")
	}
}
...
func TestLoadConfig_OpenAIWebSearchDefaultsFalseWhenUnset(t *testing.T) {
	dir := t.TempDir()
	...
	if cfg.Providers.OpenAI.WebSearch {
		t.Fatal("OpenAI codex web search should remain false when unset in config file")
	}
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	t.Setenv("NOOKCLAW_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".nookclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("NOOKCLAW_HOME", "/custom/picoclaw/home")

	cfg := DefaultConfig()
	want := "/custom/picoclaw/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with NOOKCLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
