...
import (
	...
	"github.com/samnoadd/NookClaw/pkg/migrate/internal"
	"github.com/samnoadd/NookClaw/pkg/migrate/sources/openclaw"
)
...
func (m *MigrateInstance) Run(opts Options) (*Result, error) {
	...
	fmt.Println("Migrating from Source to NookClaw")
	fmt.Printf("  Source:      %s\n", sourceHome)
	fmt.Printf("  Target: %s\n", targetHome)
	fmt.Println()
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert Source config to NookClaw format",
			})
		}
	}
