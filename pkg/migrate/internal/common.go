...
func ResolveTargetHome(override string) (string, error) {
	...
	if envHome := appenv.Getenv(appenv.HomeEnv, appenv.LegacyHomeEnv); envHome != "" {
		return ExpandHome(envHome), nil
	}
	home := appenv.DefaultHomeDir()
	if home == "" {
		return "", fmt.Errorf("resolving home directory: home directory unavailable")
	}
	return home, nil
}
