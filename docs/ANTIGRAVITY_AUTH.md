...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in NookClaw.
...
10. [Creating a New Provider in NookClaw](#creating-a-new-provider-in-nookclaw)
...
const antigravityPlugin = {
  ...
  register(api: NookClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<NookClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- NookClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.nookclaw/auth.json`:
...
## Creating a New Provider in NookClaw

NookClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add the command wiring under `cmd/nookclaw/internal/auth/`:
...
nookclaw auth login --provider your-provider

# List models (for Antigravity)
nookclaw auth models

# Start the gateway
nookclaw gateway

# Run an agent with a specific model
nookclaw agent -m "Hello" --model your-model
```
...
export NOOKCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export NOOKCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `nookclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.nookclaw/auth.json`
- Re-run `nookclaw auth login --provider antigravity`
