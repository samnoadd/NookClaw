...
NookClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, NookClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `NOOKCLAW_TOOLS_<SECTION>_<KEY>`:

For example:

- `NOOKCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `NOOKCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `NOOKCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `NOOKCLAW_TOOLS_MCP_ENABLED=true`
