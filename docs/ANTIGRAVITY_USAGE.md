# Using Antigravity Provider in NookClaw

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in NookClaw.
...
```bash
nookclaw auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where NookClaw is waiting.

NookClaw will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.nookclaw/config.json` or override it via the CLI:

```bash
# Override for a single command
nookclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `NOOKCLAW_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.nookclaw/auth.json user@your-server:~/.nookclaw/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. NookClaw will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `nookclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
