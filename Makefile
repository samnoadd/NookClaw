...
# Build variables
BINARY_NAME=nookclaw
BUILD_DIR=build
CMD_DIR=cmd/nookclaw
...
CONFIG_PKG=github.com/samnoadd/NookClaw/pkg/config
...
NOOKCLAW_HOME?=$(HOME)/.nookclaw
WORKSPACE_DIR?=$(NOOKCLAW_HOME)/workspace
...
## build: Build the nookclaw binary for current platform
build: generate
...
## build-launcher: Build the nookclaw-launcher (web console) binary
build-launcher:
	@echo "Building nookclaw-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/nookclaw-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf nookclaw-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/nookclaw-launcher
	@echo "Build complete: $(BUILD_DIR)/nookclaw-launcher"
...
## build-all: Build nookclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install nookclaw to system and copy builtin skills
install: build
...
## uninstall: Remove nookclaw from system
uninstall:
...
## uninstall-all: Remove nookclaw data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(NOOKCLAW_HOME)
	@echo "Removed workspace: $(NOOKCLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build nookclaw-agent nookclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build nookclaw-agent nookclaw-gateway
...
## docker-run: Run nookclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run nookclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run nookclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm nookclaw-agent

## docker-run-agent-full: Run nookclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm nookclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi nookclaw:latest nookclaw:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "NookClaw Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.nookclaw/workspace)"
