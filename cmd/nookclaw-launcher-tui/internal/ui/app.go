...
import (
	...
	configstore "github.com/samnoadd/NookClaw/cmd/nookclaw-launcher-tui/internal/config"
	picoclawconfig "github.com/samnoadd/NookClaw/pkg/config"
)
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open NookClaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("nookclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("nookclaw", "gateway")
