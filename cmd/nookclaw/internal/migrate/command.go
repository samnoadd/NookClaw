...
import (
	...
	"github.com/samnoadd/NookClaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to NookClaw",
		Args:  cobra.NoArgs,
		Example: `  nookclaw migrate
  nookclaw migrate --from openclaw
  nookclaw migrate --dry-run
  nookclaw migrate --refresh
  nookclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.nookclaw)")
